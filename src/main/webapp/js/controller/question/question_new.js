...
layui.use(requireModules, function (
    ...
    $("#saveSurvey").on('click', function () {
        var surveyContent = saveSurvey();

        request.request(questionNewApi.getUrl('saveQuestion'), {
            survey: JSON.stringify(surveyContent)
        }, function(result) {
            if(result.status == true){
                toast.success('保存成功');
            } else {
...
function saveSurvey() {
    var survey = {};//问卷对象
    //问卷标题
    var surveyName = $.trim($(".edit-con-title").text());

    //问卷描述
    var surveyDesc = $(".edit-box").html().replace(/[{}]/gi, "").replace(/&nbsp;/gi, " ").replace(/<br>/gi, "\n").replace(/<[^>]+>/g, "").trim("\n");
    surveyDesc = $.trim(surveyDesc);

    survey.surveyName = surveyName;
    survey.surveyDesc = surveyDesc;
    var questionList = collectContent();
...
function collectContent() {
    var questionList = [];//问题List
    $(".edit-con-box").each(function () {
        var question = {};//存储单个问题
        //问题标题
        var tmpContent = $(this).find(".edit-ques-title").find(".edit-box").html();
        var quesTitle = tmpContent.replace(/[{}]/gi, "").replace(/&nbsp;/gi, " ").replace(/<br>/gi, "\n").replace(/<[^>]+>/g, "").trim("\n");

        //问题排序
        var dataRole = $(this).find(".edit-ques-num").attr('data-role');

        //问题类型
        var quesType = $(this).find(".edit-ques-title").find(".edit-box").attr('data-type');
        //问题类型描述
        var quesName = $(this).find(".edit-ques-title").find(".edit-box").attr('data-name');
        //问题ID，新增问题没有该选项
        var questionId = $(this).find(".edit-ques-num").attr('question-id');

        question.quesTitle = quesTitle;
        question.dataRole = dataRole;
        question.quesType = quesType;
        question.quesName = quesName;
        question.questionId = questionId;
        question.choiceList = objChoice(this, quesType);        //选项数组
...
//提取选项
function objChoice(_this, quesType) {
    var choiceList = [];

    switch (quesType) {
        case 'single-type':
            $(_this).find(".ques-list").find('.form-inline').each(function (i) {
                var choice = {};
                //选项name
                var tmpChoiseTitle = $(this).find(".inline-div").find(".edit-ques-div.edit-box").html();
                var choiseTitle = tmpChoiseTitle.replace(/[{}]/gi, "").replace(/&nbsp;/gi, " ").replace(/<br>/gi, "\n").replace(/<[^>]+>/g, "").trim("\n");

                //选项value
                var dataVal = $(this).find(".inline-div").find(".edit-ques-div.edit-box").attr('data-val');
                choice.dataVal = dataVal;
                choice.choiseTitle = choiseTitle;
        ...
        case 'checkbox-type':
            ...
            break;
        case 'select-type':
            $(_this).find(".form-control.select-list option").each(function (i) {
                var choice = {};
                var optionValue = $(this).val();   //获取option值
                var channlText = $(this).text();   //获取option值

                choice.dataVal = optionValue;
                choice.choiseTitle = channlText;
                choiceList.push(choice);
            });
